...
#include "main.h" 	
#include "OLED_SPI_font.h"  
#include "OLED_SPI_bmp.h"
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				OLED_SPI_ShowChar(x+(size/2)*t,y,' ');
				continue;
			}else enshow=1; 
		 	 
		}
	 	OLED_SPI_ShowChar(x+(size/2)*t,y,temp+'0'); 
	}
} 

/// @brief //显示一个字符号串
/// @param x 列
/// @param y 行  0 - 6		1的时候显示有点问题
/// @param chr 
void OLED_SPI_ShowString(uint8_t x,uint8_t y,uint8_t *chr)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{		OLED_SPI_ShowChar(x,y,chr[j]);
			x+=8;
		if(x>120){x=0;y+=2;}
			j++;
	}
...
{      			    
    ...
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(Hzk_SPI[2*no][t],OLED_DATA);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Byte(Hzk_SPI[2*no+1][t],OLED_DATA);
				adder+=1;
      }					
}
